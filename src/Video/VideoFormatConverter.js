...
const VideoFormatConverter = () => {
  ...
  const [outputFormat, setOutputFormat] = useState('mp4'); // Default output format
  ...
  const handleConvertVideos = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:8001/convert-video', formData, {
        responseType: 'blob',
      ...
      {currentScreen === 1 && (
        <>
          ...
          <Box className="text-center d-flex justify-content-center flex-wrap" my={5}>
            ...
            Convert videos from one format to another without losing quality.
            </Typography>
          </Box>
